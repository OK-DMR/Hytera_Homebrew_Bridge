...
def read_pcap(filepath: str, hide_unknown: bool = False):
    with PcapReader(filepath) as pcap_reader:
        for pkt in pcap_reader:
            if isinstance(pkt, Ether) and pkt.haslayer(UDP.name):
                debug_udp_packet(packet=pkt, hide_unknown=hide_unknown)


if __name__ == "__main__":
    args = arguments().parse_args(sys.argv[1:])
    read_pcap(args.file, hide_unknown=args.hide_unknown)
