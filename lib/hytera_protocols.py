...
from kaitai.hytera_dmr_application_protocol import *
from kaitai.hytera_radio_network_protocol import *
from kaitai.hytera_simple_transport_reliability_protocol import *
from kaitai.ip_site_connect_heartbeat import *
from kaitai.ip_site_connect_protocol import *
from kaitai.real_time_transport_protocol import *
from lib.logging_protocol import LoggingProtocol
from lib.settings import BridgeSettings
from lib.snmp import SNMP
from tests.pcap_test import parse_hytera_data
...
class HyteraP2PProtocol(LoggingProtocol):
    ...
    def handle_rdac_request(self, data: bytes, address: tuple) -> None:
        ...
        self.log("RDAC Accept for %s.%s %s" % (address[0], address[1], hexlify(data)))
        ...
        self.log(
            ...
            % (self.settings.rdac_port, address, hexlify(data))
        )
        self.transport.sendto(data, response_address)
    ...
    def handle_dmr_request(self, data: bytes, address: tuple) -> None:
        ...
        self.transport.sendto(data, response_address)
        self.log("DMR Accept for %s.%s %s" % (address[0], address[1], hexlify(data)))

        data = self.get_redirect_packet(data, self.settings.dmr_port)

        self.log(
            "DMR Redirect to port %s response for %s %s"
            % (self.settings.dmr_port, address, hexlify(data))
        )
        self.transport.sendto(data, response_address)
...
class HyteraRDACProtocol(LoggingProtocol):
    ...
    def datagram_received(self, data: bytes, addr: Tuple[str, int]) -> None:
        if len(data) == 1 and self.step is not 14:
            if self.step == 4:
                self.log(
        ...
        elif len(data) is not 1 and self.step is 14:
            self.log("RDAC finished, received extra data %s" % hexlify(data))
        elif len(data) is 1 and self.step == 14:
            if data[0] is 0x00:
