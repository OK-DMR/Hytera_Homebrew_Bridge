...
class GenericService(Thread):
    listenPort: int
    listenIP: str = constants.DEFAULT_SERVICE_IP
    serverSocket: socket
    ...
    def set_port(self, port: int) -> GenericService:
        self.listenPort = constants.DEFAULT_SERVICE_PORT if not port else port
        self.log("Listen on Port set %s" % self.listenPort)
        return self

    def set_ip(self, ip: str) -> GenericService:
        self.listenIP = constants.DEFAULT_SERVICE_IP if not ip else ip
